...
function countVowels(str) {
  ...
  const string = str.split('').reduce((count, char) => {
...
function findLongestWord(sentence) {
  const updateString = sentence.split(' ');
  let longestWord = updateString[0];
...
function encodeToRot13(str) {
  return str.replace(/[A-Za-z]/g, (i) => {
    const base = i <= 'Z' ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
    return String.fromCharCode(((i.charCodeAt(0) - base + 13) % 26) + base);
  });
}
